            ...
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs  " data-bs-toggle="tabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <a href="{{route('int.index')}}" onclick="table_link()" id="btnTabTable" class="nav-link " data-bs-toggle="tab" aria-selected="true" role="tab">
                        ...
                        <div class="tab-pane active show " id="dataForm" role="tabpanel">      
                                <form class="form-sample" id="id_form" action="" method="POST">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="judul" class="form-control" placeholder="Nama Kerjasama">
                                    </div>
                                    <div class="form-group">
                                            <label>Gambar Lama</label><br>
                                            @if(!empty($t_international) && !empty($t_international->image_path))
                                                <img src="{{ asset('public/images/' . $t_international->image_path) }}" alt="Gambar Lama" width="200px">
                                            @else
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea rows="3" class="form-control" name="description" id="editor"> @if($t_international!=''){{$t_international->description}}@endif</textarea><br>
                                        <input type="hidden" id="txtPrimarykey" name="id" class="form-control" value="@if($t_international !='') {{$t_international->id}}@endif">
                                        <input type="hidden" id="action" class="form-control">
                                        <div style="float:right;">
                                            <a href="{{route('int.index')}}" class="btn btn-inverse-danger btn-fw btn-icon-text" id="btnCancel" onclick="actCancel()"><i class="mdi mdi-reload btn-icon-prepend"></i>&nbsp;Cancel</a>
...
<script type='text/javascript'>
    function table_link() {
        window.location.replace('{{route("int.store")}}');
    }
    $(function() {
